...
// 标记已弃用的函数
#define DEPRECATED_FUNC [[deprecated("使用 new_function 替代")]]

// 使用 #warning 提醒开发者
#warning "deprecated_api.h 已弃用，请迁移到 new_api.h"

void old_function() DEPRECATED_FUNC;
void new_function();
...
int main() {
    std::cout << "程序编译完成（尽管有警告）" << std::endl;
    return 0;
}
```
...
`#warning` 在 C++23 中正式标准化，之前作为编译器扩展存在：

```cpp
// cpp23_compliance.h
#ifndef CPP23_COMPLIANCE_H
#define CPP23_COMPLIANCE_H

// 检查 C++23 支持
#if __cplusplus >= 202302L
    #define HAS_CPP23 1
    #warning "使用 C++23 标准编译"
#else
    #define HAS_CPP23 0
    #warning "使用 C++23 之前的 C++ 标准编译"
#endif

// 条件使用 C++23 特性
#if HAS_CPP23
    // 使用 C++23 特性
    #warning "启用 C++23 特性支持"
#else
    // 回退实现
...
// 功能检测
        ...
        #warning "检测到 C++11 线程支持"
    #else
        #define HAS_THREADS 0
        #warning "未检测到 C++11 线程支持，使用替代方案"
    #endif
#else
    #define HAS_THREADS 0
...
// main.cpp
#include <iostream>
#include "cpp23_compliance.h"
#include "feature_detection.h"

int main() {
    std::cout << "C++23 合规性检查完成" << std::endl;
    return 0;
}
```
...
// 生成详细的诊断消息
#if __cplusplus < 201103L
    #error "需要 C++11 或更高版本。当前编译器: " COMPILER_INFO ", 平台: " PLATFORM_NAME
#endif
...
int main() {
    std::cout << "诊断消息格式化示例" << std::endl;
    std::cout << "项目版本: " << PROJECT_VERSION_STRING << std::endl;
    return 0;
}
```
...
int main() {
    std::cout << "平台兼容性检查通过" << std::endl;
    std::cout << "目标平台: " << TARGET_OS << " (" << TARGET_ARCH << ")" << std::endl;
    return 0;
}
```
...
// C++23 #warning 支持检查
#if __cplusplus >= 202302L
    #define CPP23_WARNING_SUPPORTED 1
    #warning "使用 C++23 标准，#warning 正式支持"
#else
    #define CPP23_WARNING_SUPPORTED 0
    #warning "使用 C++23 之前的 C++ 标准，#warning 作为扩展支持"
#endif
...
// 根据 C++ 标准版本启用/禁用功能
#if HAS_CPP23_FEATURES
    #warning "启用 C++23 特性"
    // 使用 C++23 特性
    
#elif HAS_CPP20_FEATURES
    #warning "启用 C++20 特性"
    // 使用 C++20 特性
    
#elif HAS_CPP17_FEATURES
    #warning "启用 C++17 特性"
    // 使用 C++17 特性
    
#elif HAS_CPP14_FEATURES
    #warning "启用 C++14 特性"
    // 使用 C++14 特性
    
#elif HAS_CPP11_FEATURES
    #warning "启用 C++11 特性"
    // 使用 C++11 特性
    
#else
    #warning "使用 C++98/C++03 兼容模式"
    // 使用兼容模式实现
#endif

// 功能回退处理
#if !HAS_CPP11_FEATURES
    #warning "C++11 特性不可用，使用回退实现"
    // 提供 C++98/C++03 兼容的实现
#endif
...
int main() {
    std::cout << "C++ 标准版本: " << CPP_STANDARD << " (" << CPP_STANDARD_YEAR << ")" << std::endl;
    std::cout << "版本兼容性检查完成" << std::endl;
    return 0;
}
```
...
// 线程支持检查
        ...
        #warning "支持 C++11 线程"
    #else
        #warning "C++11 标准但禁用了线程支持"
    #endif
#else
    #warning "C++11 之前的标准，使用平台特定线程"
#endif
...
- C++98/C++03 (ISO/IEC 14882:1998/2003)：定义了 `#error` 指令的基本语法和语义
- C++11 (ISO/IEC 14882:2011)：保持 `#error` 不变，未引入新特性
- C++14 (ISO/IEC 14882:2014)：保持 `#error` 不变，未引入新特性
- C++17 (ISO/IEC 14882:2017)：保持 `#error` 不变，未引入新特性
- C++20 (ISO/IEC 14882:2020)：保持 `#error` 不变，未引入新特性
- C++23 (ISO/IEC 14882:2024)：正式标准化 `#warning` 指令，使其成为标准特性

### 8.2 相关标准条款

**C++23, C++20, C++17, C++14, C++11, C++98**：

- 15.8 Error directive [cpp.error]：详细描述了 `#error` 指令的语法、语义和处理过程
- (C++23新增) 15.9 Warning directive [cpp.warning]：描述了 `#warning` 指令的语法和语义
...
// C++23 标准合规性检查
    ...
    #warning "完全符合 C++23 标准"
#else
    #define CPP23_COMPLIANT 0
    #warning "使用 C++23 之前的 C++ 标准"
#endif
...
// C++20 标准合规性检查
    ...
    #warning "使用 C++20 之前的 C++ 标准"
#endif

// C++17 标准合规性检查
#if __cplusplus >= 201703L
    #define CPP17_COMPLIANT 1
    #if CPP20_COMPLIANT || CPP23_COMPLIANT
        // 更高版本包含 C++17 特性
    #else
        #warning "符合 C++17 标准"
    #endif
...
// 编译器特定标准支持
        ...
        #if __cplusplus >= 202302L
            #warning "Clang 完全支持 C++23"
        #elif __cplusplus >= 202002L
            #warning "Clang 支持 C++20"
        #elif __cplusplus >= 201703L
            #warning "Clang 支持 C++17"
        #elif __cplusplus >= 201402L
            #warning "Clang 支持 C++14"
        #elif __cplusplus >= 201103L
            #warning "Clang 支持 C++11"
        #else
            #warning "Clang 支持 C++98/C++03"
        #endif
    #else
        ...
        #if __cplusplus >= 202002L
            #warning "MSVC 支持 C++20"
        #elif __cplusplus >= 201703L
            #warning "MSVC 支持 C++17"
        #elif __cplusplus >= 201402L
            #warning "MSVC 支持 C++14 (部分)"
        #elif __cplusplus >= 201103L
            #warning "MSVC 支持 C++11"
        #else
            #warning "MSVC 支持 C++98/C++03"
        #endif
    #else
...
- 条件使用：通常与条件编译指令结合使用
- C++23 标准化：`#warning` 在 C++23 中正式标准化
