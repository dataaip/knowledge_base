...
这是 C 语言标准规定，在编写源代码时必须支持的最小字符集合，包含 95 个可打印字符和若干控制字符。也就是说，任何一个符合标准的 C 编译器都必须能够识别和处理包含这些字符的源代码文件。
...
这是在程序运行时，环境必须支持的最小字符集合。它包含了“基本字符集”的所有字符，并额外增加了几个控制字符。
...
- 特点：每个 wchar_t 变量或数组元素通常对应一个“逻辑”字符。字符串操作变得简单，类似于处理 ASCII 字符串（wcslen() 可以准确返回宽字符的个数）。浪费空间，尤其是对于以ASCII字符为主的文本。
