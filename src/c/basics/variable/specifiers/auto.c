...
* <tr><td>2024.12.24 <td>0.1             <td>brightl         <td>auto关键词使用，解释说明
* </table>
*/
...
int auto_fn(void) {
  ...
  register 说明符只允许用于声明于块作用域的对象，包括函数形参列表。它指示自动存储期与无链接（即这种声明的默认属性），但另外提示优化器，若可能则将此对象的值存储于 CPU 寄存器中。无论此优化是否发生，声明为 register 的对象不能用作取址运算符的参数，不能用 alignas(C11起)，而且 register 数组不能转换为指针
