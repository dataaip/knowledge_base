...
- **文本文件**：`.cpp`源文件包含实现，`.hpp`头文件包含声明（C++20模块可替代）
- **翻译过程**：
  1. 预处理：处理`#include`和宏（`g++ -E main.cpp -o main.ii`）
  2. 编译：生成汇编（支持C++特性如模板实例化）
  3. 汇编：生成目标文件
  4. 链接：解析符号引用（包括C++名称修饰后的符号）
- **执行入口**：标准规定从`main`函数开始（全局对象的构造函数在`main`前执行）
...
- **关键词**：C++20有92个保留词（如`class`, `template`, `concept`）
- **标识符规则**：
  - 支持Unicode标识符（C++11起）
  - 不能以数字开头，不能包含运算符字符（如@, $）
- **注释**：`//`单行注释，`/* */`多行注释（编译前被移除）
...
- **实体类型**：
  - **值**：表达式结果（如`2+2`的值）
  - **对象**：存储区域（含类实例）
  - **引用**：`int&`（左值引用），`int&&`（右值引用）
  - **结构化绑定**（C++17）：解构对象成员
  - **函数**：普通函数、成员函数、lambda
  - **类型**：基础类型、类、枚举
  - **模板**：函数模板、类模板
  - **参数包**（C++11）：变长模板参数`template <typename... Ts>`
- **非实体**：预处理器宏（在编译前被替换）
