...
每个抽象字符在 Unicode 代码空间中分配了一个码位，并且
未分配给抽象字符的每个 Unicode 标量值的不同字符。
翻译字符集是基本字符集和基本文字字符集的超集（见下文）。

（自 C++23 起）
基本字符集
基本字符集由以下 96 个字符（直到 C++26）99（自 C++26 起）字符组成：
...
U+000D	回车 （CR）
执行字符集
执行字符集和执行宽字符集是基本文本的超集 字符集。执行字符集和其他元素集的编码 （如果有）是特定于区域设置的。执行宽字符集的每个元素都必须可表示为不同的 wchar_t 代码单元。
