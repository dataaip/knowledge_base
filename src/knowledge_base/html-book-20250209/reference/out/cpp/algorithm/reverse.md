...
1) Reverses the order of the elements in the range ``first`,`last`)`. Behaves as if applying [std::iter_swap to every pair of iterators first + i and (last - i) - 1 for each integer i in ``​0​`,`[std::distance(first, last) / 2`)`.2) Same as (1), but executed according to policy. This overload participates in overload resolution only if all following conditions are satisfied:
...
Retrieved from "<https://en.cppreference.com/mwiki/index.php?title=cpp/algorithm/reverse&oldid=170513>"
...
- This page was last modified on 27 March 2024, at 18:58.