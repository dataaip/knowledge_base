...
1) Public member function, calls the protected virtual member function `do_hash` of the most derived class.2) Converts the character sequence ``beg`,`end`)` to an integer value that is equal to the hash obtained for all strings that collate equivalent in this locale ([compare() returns ​0​). For two strings that do not collate equivalent, the probability that their hashes are equal should be very small, approaching 1.0 / std::numeric_limits<unsigned long>::max().
...
- This page was last modified on 3 October 2023, at 06:55.