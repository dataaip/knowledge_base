...
| public:  bool is( mask m, CharT c ) const; | (1) |  |
| public:  const CharT\* is( const CharT\* low, const CharT\* high, mask\* vec ) const; | (2) |  |
| protected:  virtual bool do_is( mask m, CharT c ) const; | (3) |  |
| protected:  virtual const CharT\* do_is( const CharT\* low, const CharT\* high, mask\* vec ) const; | (4) |  |
|  |  |  |

1,2) Public member function, calls the protected virtual member function `do_is` of the most derived class.3) Checks if the character c is classified by the mask m.4) For every character in the character array `[`low`,`high`)`, identifies the complete classification mask (e.g. digit|xdigit|alnum|print|graph for the digit '0' in the default locale), and stores the masks in the corresponding elements of the array pointed to by vec.
...
1,3) true if c is classified by m.2,4) high
...
- This page was last modified on 4 October 2023, at 05:11.