...
1) If `T` is a specialization of std::reference_wrapper, unwraps it; otherwise, `T` remains the same.2) If the decayed `T` is a specialization of std::reference_wrapper, unwraps it; otherwise, `T` is decayed.
...
| --- | --- |
| Type | Definition |
| `type` | (1) `U&` if `T` is std::reference_wrapper<U>; `T` otherwise  (2) `U&` if std::decay_t<T> is std::reference_wrapper<U>; std::decay_t<T> otherwise |
...
|  |
| --- |
| ```text template<class T> struct unwrap_reference { using type = T; }; template<class U> struct unwrap_reference<std::reference_wrapper<U>> { using type = U&; };   template<class T> struct unwrap_ref_decay : std::unwrap_reference<std::decay_t<T>> {}; ``` |
...
- This page was last modified on 4 November 2024, at 18:11.