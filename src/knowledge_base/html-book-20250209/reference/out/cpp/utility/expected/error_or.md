...
1) If std::is_copy_constructible_v<E> or std::is_convertible_v<G, E> is false, the program is ill-formed.2) If std::is_move_constructible_v<E> or std::is_convertible_v<G, E> is false, the program is ill-formed.
...
1) has_value() ? std::forward<G>(default_value) : error()2) has_value() ? std::forward<G>(default_value) : std::move(error())
...
Retrieved from "<https://en.cppreference.com/mwiki/index.php?title=cpp/utility/expected/error_or&oldid=173321>"
...
- This page was last modified on 5 July 2024, at 01:44.