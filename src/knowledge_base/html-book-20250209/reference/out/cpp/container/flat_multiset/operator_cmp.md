...
|  |  |  |
| --- | --- | --- |
| friend bool operator==( const std::flat_multiset& lhs,                          const std::flat_multiset& rhs ); | (1) | (since C++23) |
| friend synth-three-way-result<value_type>      operator<=>( const std::flat_multiset& lhs, const std::flat_multiset& rhs ); | (2) | (since C++23) |
|  |  |  |
...
1) Constant if lhs and rhs are of different size, otherwise linear in the size of the `flat_multiset`.2)
...
Retrieved from "<https://en.cppreference.com/mwiki/index.php?title=cpp/container/flat_multiset/operator_cmp&oldid=169574>"
