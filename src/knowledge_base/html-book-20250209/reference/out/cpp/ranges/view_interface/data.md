...
|  |  |  |
| --- | --- | --- |
| constexpr auto data()      requires std::contiguous_iterator<ranges::iterator_t<D>>; | (1) | (since C++20) |
| constexpr auto data() const      requires ranges::range<const D> && std::contiguous_iterator<ranges::iterator_t<const D>>; | (2) | (since C++20) |
|  |  |  |
...
1) Let `derived` be static_cast<D&>(\*this). Equivalent to return std::to_address(ranges::begin(derived));.2) Same as (1), except that `derived` is static_cast<const D&>(\*this).
...
Retrieved from "<https://en.cppreference.com/mwiki/index.php?title=cpp/ranges/view_interface/data&oldid=137550>"
...
- This page was last modified on 26 January 2022, at 17:12.