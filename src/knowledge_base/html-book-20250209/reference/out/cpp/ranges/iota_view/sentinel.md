...
1) Value-initializes `bound_` ﻿.2) Initializes `bound_` with bound.
...
1) Returns x.`value_`- y.`bound_`.2) Returns -(y.`value_`- x.`bound_` ﻿).
...
- This page was last modified on 8 October 2024, at 08:59.