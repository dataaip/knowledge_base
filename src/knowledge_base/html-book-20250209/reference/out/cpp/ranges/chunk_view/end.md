...
1) Available only if V models `input_range`. Equivalent to: return std::default_sentinel.2,3) Available if V models `forward_range`. Let `base_` denote the underlying adapted view, `n_` denote the stored chunk size, and `iterator` denote the nested iterator class.2) Equivalent to:
...
Retrieved from "<https://en.cppreference.com/mwiki/index.php?title=cpp/ranges/chunk_view/end&oldid=160868>"
...
- This page was last modified on 17 October 2023, at 21:38.