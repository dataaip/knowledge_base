...
|  |  |  |
| --- | --- | --- |
| transform_view() requires std::default_initializable<V> &&                            std::default_initializable<F> = default; | (1) | (since C++20) |
| constexpr explicit transform_view( V base, F fun ); | (2) | (since C++20) |
|  |  |  |

Constructs a `transform_view`.

1) Default constructor. Value-initializes the underlying view `base_` and the transformation function `fun_`.2) Move constructs the underlying view `base_` from `base` and the transformation function `fun_` from `fun`.
...
| P2325R3 | C++20 | if `F` is not `default_initializable`, the default constructor constructs a `transform_view` which does not contain an `F` | the `transform_view` is also not `default_initializable` |

Retrieved from "<https://en.cppreference.com/mwiki/index.php?title=cpp/ranges/transform_view/transform_view&oldid=173539>"
...
- This page was last modified on 13 July 2024, at 01:26.