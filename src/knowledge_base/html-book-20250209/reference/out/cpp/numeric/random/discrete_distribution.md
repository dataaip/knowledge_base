...
`std::discrete_distribution` produces random integers on the interval ``​0​`,`n`)`, where the probability of each individual integer `i` is defined as wi/S, that is the **weight** of the `i`th integer divided by the sum of all `n` weights.
...
| Characteristics | |
| probabilities | obtains the list of probabilities   (public member function) |
...
Retrieved from "<https://en.cppreference.com/mwiki/index.php?title=cpp/numeric/random/discrete_distribution&oldid=151165>"
...
- This page was last modified on 29 April 2023, at 07:16.