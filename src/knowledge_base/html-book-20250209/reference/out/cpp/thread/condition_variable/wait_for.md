...
1) Equivalent to return wait_until(lock, std::chrono::steady_clock::now() + rel_time);.2) Equivalent to return wait_until(lock, std::chrono::steady_clock::now() + rel_time, std::move(pred));. This overload may be used to ignore spurious awakenings while waiting for a specific condition to become true.

Right after `wait_for` returns, lock.owns_lock() is true, and lock.mutex() is locked by the calling thread. If these postconditions cannot be satisfied[[1]](wait_for.html#cite_note-1), calls std::terminate.
...
1) std::cv_status::timeout if rel_time has been elapsed since the beginning of this call, otherwise std::cv_status::no_timeout.2) The latest result of pred() before returning to the caller.
...
Retrieved from "<https://en.cppreference.com/mwiki/index.php?title=cpp/thread/condition_variable/wait_for&oldid=176498>"
...
- This page was last modified on 25 September 2024, at 04:32.