...
混合方法通过结合多种解析技术的优势，在保证性能的同时增强扩展性，适用于需要支持复杂表达式（如函数调用、括号嵌套、变量作用域）的科学计算器。以下是三种典型的混合实现方式：

### 1. 栈 + 递归

**核心思想**：

- 主表达式用栈处理：使用双栈（操作数栈 + 运算符栈）处理基本算术运算

- 函数参数递归计算：遇到函数调用（如 sin(2+3)）时，递归解析参数部分

**实现步骤**：

- 词法分析：生成 Token 流（数字、运算符、函数名、括号等）

- 主循环（栈处理）：遇到数字或变量压入操作数栈、遇到运算符按优先级和结合性处理（类似 Shunting-Yard 算法）

- 遇到函数：（如 sin）递归解析括号内参数，结果压入操作数栈

- 递归求值函数参数

**特点**：

- 优点：函数调用支持灵活（如 max(1, 2+3)）、栈处理保证主表达式的高效性（O(n)）

- 缺点：递归可能引发栈溢出（需限制递归深度）

### 2. AST + 访客模式

**核心思想**：

- 构建抽象语法树（AST）：将表达式转换为树形结构，明确运算顺序

- 访客模式求值：通过遍历 AST 动态执行计算逻辑，分离解析与求值
...
- 访客模式求值：递归访问左子树 → 递归访问右子树 → 查表执行运算

**特点**：

- 优点：支持复杂表达式（如变量、函数、作用域）、易扩展新运算符或语法（只需修改 AST 生成规则）

- 缺点：内存占用较高（需存储 AST）、实现复杂度较高

### 3. 递归下降 + 优先级爬升

**核心思想**：

- 递归下降：分层次解析表达式（如 表达式 → 项 → 因子）
- 优先级爬升：动态处理运算符优先级，避免显式栈

**实现步骤**：

- 词法分析：生成 Token 流

- 递归解析：表达式层处理加减法（左结合）、项层处理乘除法（更高优先级）、因子层：处理数字、括号、函数调用

- 优先级爬升：解析左侧操作数、递归处理高优先级

**特点**：

- 优点：无显式栈，代码简洁、天然支持优先级和结合性
