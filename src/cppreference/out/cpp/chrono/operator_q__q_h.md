...
| constexpr std::chrono::hours      operator""h( unsigned long long hrs ); | (1) | (since C++14) |
| constexpr std::chrono::duration</\*unspecified\*/, std::ratio<3600,1>>      operator""h( long double hrs ); | (2) | (since C++14) |
|  |  |  |

Forms a std::chrono::duration literal representing hours.

1) Integer literal, returns exactly std::chrono::hours(hrs).2) Floating-point literal, returns a floating-point duration equivalent to std::chrono::hours.
...
|  |
| --- |
| ```text constexpr std::chrono::hours operator""h(unsigned long long h) {     return std::chrono::hours(h); }   constexpr std::chrono::duration<long double, ratio<3600,1>> operator""h(long double h) {     return std::chrono::duration<long double, std::ratio<3600,1>>(h); } ``` |
...
Retrieved from "<https://en.cppreference.com/mwiki/index.php?title=cpp/chrono/operator%22%22h&oldid=157627>"
...
- This page was last modified on 31 August 2023, at 12:51.