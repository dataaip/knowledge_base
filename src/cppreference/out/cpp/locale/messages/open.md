...
| public:  catalog open( const std::string& name, const std::locale& loc ) const; | (1) |  |
| protected:  virtual catalog do_open( const std::string& name, const std::locale& loc ) const; | (2) |  |
|  |  |  |

1) Public member function, calls the protected virtual member function `do_open` of the most derived class.2) Obtains a value of type `catalog` (inherited from std::messages_base), which can be passed to get() to retrieve messages from the message catalog named by name. This value is usable until passed to close().
...
- This page was last modified on 20 January 2025, at 07:02.