...
1) Let derived be a reference bound to static_cast<D&>(\*this). Equivalent to return ranges::size(derived) == 0; if `D` satisfies `sized_range`. Otherwise, equivalent to return ranges::begin(derived) == ranges::end(derived);.2) Same as (1), except that derived is static_cast<const D&>(\*this).
...
Retrieved from "<https://en.cppreference.com/mwiki/index.php?title=cpp/ranges/view_interface/empty&oldid=162841>"
...
- This page was last modified on 15 November 2023, at 11:04.