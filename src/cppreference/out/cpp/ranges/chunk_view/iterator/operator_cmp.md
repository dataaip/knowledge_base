...
| friend constexpr auto operator<=>( const /\*iterator\*/& x, const /\*iterator\*/& y )      requires ranges::random_access_range<Base> && std::three_way_comparable<ranges::iterator_t<Base>>; | (7) | (since C++23) |
|  |  |  |
...
1) return x.current_ == y.current_;.2) return x.current_ == x.end_;.3) return x.current_ < y.current_;.4) return y < x;.5) return !(y < x);.6) return !(x < y);.7) return x.current_ <=> y.current_;.
...
Retrieved from "<https://en.cppreference.com/mwiki/index.php?title=cpp/ranges/chunk_view/iterator/operator_cmp&oldid=172666>"
...
- This page was last modified on 20 June 2024, at 21:12.