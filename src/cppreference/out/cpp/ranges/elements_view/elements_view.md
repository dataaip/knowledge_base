...
|  |  |  |
| --- | --- | --- |
| elements_view() requires std::default_initializable<V> = default; | (1) | (since C++20) |
| constexpr explicit elements_view( V base ); | (2) | (since C++20) |
|  |  |  |

Constructs an `elements_view`.

1) Default constructor. Value-initializes the underlying view `base_`. After construction, `base()` returns a copy of V().2) Initializes the underlying view `base_` with std::move(base).
...
Retrieved from "<https://en.cppreference.com/mwiki/index.php?title=cpp/ranges/elements_view/elements_view&oldid=173586>"
...
- This page was last modified on 13 July 2024, at 11:06.