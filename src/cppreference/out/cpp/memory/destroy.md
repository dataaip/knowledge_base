...
| template< class ForwardIt >  constexpr void destroy( ForwardIt first, ForwardIt last ); |  | (since C++20) |
|  |  |  |
| --- | --- | --- |
...
|  |
| --- |
| ```text template<class ForwardIt> constexpr // since C++20 void destroy(ForwardIt first, ForwardIt last) {     for (; first != last; ++first)         std::destroy_at(std::addressof(*first)); } ``` |
...
Retrieved from "<https://en.cppreference.com/mwiki/index.php?title=cpp/memory/destroy&oldid=153302>"
...
- This page was last modified on 13 June 2023, at 09:39.