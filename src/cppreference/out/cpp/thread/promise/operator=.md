...
1) Move assignment operator. First, abandons the shared state (as in ~promise()), then assigns the shared state of other as if by executing std::promise(std::move(other)).swap(\*this).2) `promise` is not copy-assignable.
...
Retrieved from "<https://en.cppreference.com/mwiki/index.php?title=cpp/thread/promise/operator%3D&oldid=160653>"
...
- This page was last modified on 13 October 2023, at 22:14.