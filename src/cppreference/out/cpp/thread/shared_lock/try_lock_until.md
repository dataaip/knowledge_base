...
|  |  |  |
| --- | --- | --- |
| template< class Clock, class Duration >  bool try_lock_until( const std::chrono::time_point<Clock,Duration>& timeout_time ); |  | (since C++14) |
|  |  |  |
...
| try_lock_until | tries to lock (i.e., takes ownership of) the associated TimedLockable mutex, returns if the mutex has been unavailable until specified time point has been reached   (public member function of `std::unique_lock<Mutex>`) |
...
- This page was last modified on 23 October 2023, at 10:03.