...
1) Default constructor. Constructs a `std::future` with no shared state. After construction, valid() == false.2) Move constructor. Constructs a `std::future` with the shared state of other using move semantics. After construction, other.valid() == false.3) `std::future` is not CopyConstructible.
...
Retrieved from "<https://en.cppreference.com/mwiki/index.php?title=cpp/thread/future/future&oldid=161132>"
...
- This page was last modified on 22 October 2023, at 09:00.