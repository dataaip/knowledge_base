...
1) Constructs an empty `stop_token` with no associated stop-state.2) Copy constructor. Constructs a `stop_token` whose associated stop-state is the same as that of other.3) Move constructor. Constructs a `stop_token` whose associated stop-state is the same as that of other; other is left empty.
...
1) stop_possible() and stop_requested() are both false.2) \*this and other share the same associated stop-state and compare equal.3) \*this has other's previously associated stop-state, and other.stop_possible() is false.
Retrieved from "<https://en.cppreference.com/mwiki/index.php?title=cpp/thread/stop_token/stop_token&oldid=161275>"
...
- This page was last modified on 23 October 2023, at 23:43.