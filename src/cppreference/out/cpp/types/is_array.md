...
|  |  |  |
| --- | --- | --- |
| template< class T >  constexpr bool is_array_v = is_array<T>::value; |  | (since C++17) |
|  |  |  |
...
|  |  |
| --- | --- |
| value[static] | true if `T` is an array type, false otherwise   (public static member constant) |
...
|  |
| --- |
| ```text template<class T> struct is_array : std::false_type {};   template<class T> struct is_array<T[]> : std::true_type {};   template<class T, std::size_t N> struct is_array<T[N]> : std::true_type {}; ``` |
...
- This page was last modified on 24 September 2024, at 04:51.