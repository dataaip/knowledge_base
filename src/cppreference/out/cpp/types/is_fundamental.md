...
| template< class T >  struct is_fundamental; |  | (since C++11) |
|  |  |  |
...
|  |  |  |
| --- | --- | --- |
| template< class T >  constexpr bool is_fundamental_v = is_fundamental<T>::value; |  | (since C++17) |
|  |  |  |
...
|  |  |
| --- | --- |
| value[static] | true if `T` is a fundamental type, false otherwise   (public static member constant) |
...
|  |
| --- |
| ```text template<class T> struct is_fundamental     : std::integral_constant<         bool,         std::is_arithmetic<T>::value ||         std::is_void<T>::value ||         std::is_same<std::nullptr_t, typename std::remove_cv<T>::type>::value         // you can also use 'std::is_null_pointer<T>::value' instead in C++14 > {}; ``` |
...
Retrieved from "<https://en.cppreference.com/mwiki/index.php?title=cpp/types/is_fundamental&oldid=176472>"
...
- This page was last modified on 24 September 2024, at 13:38.