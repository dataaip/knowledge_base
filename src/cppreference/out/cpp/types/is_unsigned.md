...
|  |  |
| --- | --- |
| value[static] | true if `T` is an unsigned integral type, false otherwise   (public static member constant) |
...
|  |
| --- |
| ```text namespace detail {     template<typename T,bool = std::is_arithmetic<T>::value>     struct is_unsigned : std::integral_constant<bool, T(0) < T(-1)> {};       template<typename T>     struct is_unsigned<T,false> : std::false_type {}; } // namespace detail   template<typename T> struct is_unsigned : detail::is_unsigned<T>::type {}; ``` |
...
Retrieved from "<https://en.cppreference.com/mwiki/index.php?title=cpp/types/is_unsigned&oldid=177625>"
...
- This page was last modified on 12 November 2024, at 21:56.