...
| bool is_character_file( const std::filesystem::path& p, std::error_code& ec ) noexcept; | (3) | (since C++17) |
|  |  |  |
...
1) Equivalent to s.type() == file_type::character.2,3) Equivalent to is_character_file(status(p)) or is_character_file(status(p, ec)) respectively.
...
| is_character_file | checks whether the directory entry refers to a character device   (public member function of `std::filesystem::directory_entry`) |

Retrieved from "<https://en.cppreference.com/mwiki/index.php?title=cpp/filesystem/is_character_file&oldid=158207>"
...
- This page was last modified on 8 September 2023, at 07:42.