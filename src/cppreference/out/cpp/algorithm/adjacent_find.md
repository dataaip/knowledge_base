...
1) Exactly \(\scriptsize \min(M+1,N-1)\)min(M+1,N-1) comparisons using operator==.2) \(\scriptsize O(N)\)O(N) comparisons using operator==.3) Exactly \(\scriptsize \min(M+1,N-1)\)min(M+1,N-1) applications of the predicate p.4) \(\scriptsize O(N)\)O(N) applications of the predicate p.
...
| adjacent_find (1) |
| --- |
| ```text template<class ForwardIt> ForwardIt adjacent_find(ForwardIt first, ForwardIt last) {     if (first == last)         return last;       ForwardIt next = first;     ++next;       for (; next != last; ++next, ++first)         if (*first == *next)             return first;       return last; } ``` |
| adjacent_find (3) |
| ```text template<class ForwardIt, class BinaryPred> ForwardIt adjacent_find(ForwardIt first, ForwardIt last, BinaryPred p) {     if (first == last)         return last;       ForwardIt next = first;     ++next;       for (; next != last; ++next, ++first)         if (p(*first, *next))             return first;       return last; } ``` |
...
Retrieved from "<https://en.cppreference.com/mwiki/index.php?title=cpp/algorithm/adjacent_find&oldid=170559>"
...
- This page was last modified on 1 April 2024, at 22:38.