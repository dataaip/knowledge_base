...
1) The non-specialized `default_delete` uses delete to deallocate memory for a single object.2) A partial specialization for array types that uses delete[] is also provided.
...
1) Constructs a `std::default_delete` object.2) Constructs a `std::default_delete<T>` object from another `std::default_delete` object. This overload participates in overload resolution only if `U*` is implicitly convertible to `T*`.3) Constructs a `std::default_delete<T[]>` object from another `std::default_delete<U[]>` object. This overload participates in overload resolution only if `U(*)[]` is implicitly convertible to `T(*)[]`.
...
1) Calls delete on ptr.2) Calls delete[] on ptr. This overload participates in overload resolution only if `U(*)[]` is implicitly convertible to `T(*)[]`. If `U` is an incomplete type, the program is ill-formed.
...
- This page was last modified on 3 January 2025, at 03:13.