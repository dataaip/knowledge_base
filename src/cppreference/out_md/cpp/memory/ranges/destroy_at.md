...
| Call signature |  |  |
| template< std::destructible T >  constexpr void destroy_at( T\* p ) noexcept; |  | (since C++20) |
|  |  |  |
...
|  |
| --- |
| ```text struct destroy_at_fn {     template<std::destructible T>     constexpr void operator()(T *p) const noexcept     {         if constexpr (std::is_array_v<T>)             for (auto &elem : *p)                 operator()(std::addressof(elem));         else             p->~T();     } };   inline constexpr destroy_at_fn destroy_at{}; ``` |
...
Retrieved from "<https://en.cppreference.com/mwiki/index.php?title=cpp/memory/ranges/destroy_at&oldid=153427>"
...
- This page was last modified on 15 June 2023, at 07:41.