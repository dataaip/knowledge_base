...
1,2) Effectively calls rdbuf()->open(filename, mode | ios_base::in) (see std::basic_filebuf::open for the details on the effects of that call). Overload (2) is only provided if `std::filesystem::path::value_type` is not char.(since C++17)3,4) Effectively calls (1,2) as if by open(filename.c_str(), mode).
...
Retrieved from "<https://en.cppreference.com/mwiki/index.php?title=cpp/io/basic_ifstream/open&oldid=147679>"
...
- This page was last modified on 8 February 2023, at 19:25.