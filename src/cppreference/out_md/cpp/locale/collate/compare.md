...
1) Public member function, calls the protected virtual member function `do_compare` of the most derived class.2) Compares the character sequence ``low1`,`high1`)` to the character sequence `[`low2`,`high2`)`, using this locale's collation rules, and returns 1 if the first string follows the second, -1 if the first string precedes the second, zero if the two strings are equivalent.
...
- This page was last modified on 3 October 2023, at 06:45.