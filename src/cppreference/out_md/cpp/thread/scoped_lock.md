...
| `mutex_type` (conditionally present) | If sizeof...(MutexTypes) == 1, member type `mutex_type` is the same as `Mutex`, the sole type in `MutexTypes...`. Otherwise, there is no member `mutex_type`. |
...
Retrieved from "<https://en.cppreference.com/mwiki/index.php?title=cpp/thread/scoped_lock&oldid=173481>"
...
- This page was last modified on 12 July 2024, at 00:07.