...
1) Atomically calls lock.unlock() and blocks on \*this. The thread will be unblocked when notify_all() or notify_one() is executed. It may also be unblocked spuriously. When unblocked, calls lock.lock() (possibly blocking on the lock), then returns.2) Equivalent to while (!pred())  
    wait(lock);. This overload may be used to ignore spurious awakenings while waiting for a specific condition to become true.

Right after `wait` returns, lock.owns_lock() is true, and lock.mutex() is locked by the calling thread. If these postconditions cannot be satisfied[[1]](wait.html#cite_note-1), calls std::terminate.
...
1) Throws nothing.2) Any exception thrown by pred.
...
Retrieved from "<https://en.cppreference.com/mwiki/index.php?title=cpp/thread/condition_variable/wait&oldid=178570>"
...
- This page was last modified on 20 December 2024, at 13:32.