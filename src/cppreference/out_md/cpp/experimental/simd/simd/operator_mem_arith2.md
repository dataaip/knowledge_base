...
1) Returns a simd_mask<T, Abi> where the ith element equals !operator[](i) for all i in the range of ``​0​`,`[`size()``)`.2) Returns a `simd` where each bit is the inverse of the corresponding bit in \*this. This overload participates in overload resolution only if `T` is an integral type.3) Returns a copy of itself.4) Returns a `simd` where the ith element is initialized to -operator[](i) for all i in the range of ``​0​`,`[`size()``)`.
...
- This page was last modified on 2 October 2023, at 06:26.