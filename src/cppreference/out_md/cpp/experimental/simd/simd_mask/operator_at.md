...
| --- | --- | --- | --- | --- |
| Member functions | | | | |
| |  |  |  |  |  | | --- | --- | --- | --- | --- | | simd_mask | | | | | | size | | | | | | |  |  |  |  |  | | --- | --- | --- | --- | --- | | copy_from | | | | | | copy_to | | | | | | |  |  |  |  |  | | --- | --- | --- | --- | --- | | ****operator[]**** | | | | | | operator!") | | | | | |
...
1) Returns a reference proxy to the i-th element. This proxy type should not be captured into an lvalue. Lvalues of `simd_mask::reference` can only convert to `value_type`. Rvalues of `simd_mask::reference` overload assignment and all compound assignment operators as well as `swap`.2) Returns a prvalue of the i-th element. In contrast to containers, which contain objects of type `value_type`, a `simd_mask` is not a container of individual objects and therefore cannot return an lvalue-reference.
...
Retrieved from "<https://en.cppreference.com/mwiki/index.php?title=cpp/experimental/simd/simd_mask/operator_at&oldid=159780>"
...
- This page was last modified on 28 September 2023, at 10:09.