...
swap(std::declval<U>(), std::declval<T>()) are both well-formed in unevaluated context after using std::swap; (see Swappable), provides the member constant `value` equal true. Otherwise, `value` is false. Access checks are performed as if from a context unrelated to either type.3) Same as (1), but evaluations of both expressions from (1) are known not to throw exceptions.
...
- This page was last modified on 5 April 2024, at 12:10.