...
|  |  |  |
| --- | --- | --- |
| template< class T >  constexpr bool is_integral_v = is_integral<T>::value; |  | (since C++17) |
|  |  |  |
...
|  |  |
| --- | --- |
| value[static] | true if `T` is an integral type, false otherwise   (public static member constant) |
...
|  |
| --- |
| ```text // Note: this implementation uses C++20 facilities template<class T> struct is_integral : std::bool_constant<     requires (T t, T* p, void (*f)(T)) // T* parameter excludes reference types     {         reinterpret_cast<T>(t); // Exclude class types         f(0); // Exclude enumeration types         p + t; // Exclude everything not yet excluded but integral types     }> {}; ``` |
...
Retrieved from "<https://en.cppreference.com/mwiki/index.php?title=cpp/types/is_integral&oldid=169677>"
...
- This page was last modified on 8 February 2024, at 21:34.