...
| template< class RandomIt >  void push_heap( RandomIt first, RandomIt last ); | (1) | (constexpr since C++20) |
| template< class RandomIt, class Compare >  void push_heap( RandomIt first, RandomIt last, Compare comp ); | (2) | (constexpr since C++20) |
|  |  |  |

Inserts the element at the position last - 1 into the heap ``first`,`last - 1`)`. The heap after the insertion will be `[`first`,`last`)`.

1) The heap is with respect to operator<(until C++20)[std::less{}(since C++20).2) The heap is with respect to comp.
...
1) At most \(\scriptsize \log(N)\)log(N) comparisons using operator<(until C++20)std::less{}(since C++20).2) At most \(\scriptsize \log(N)\)log(N) applications of the comparison function comp.
...
Retrieved from "<https://en.cppreference.com/mwiki/index.php?title=cpp/algorithm/push_heap&oldid=177048>"
...
- This page was last modified on 13 October 2024, at 22:13.