...
| template< class BidirIt >  bool next_permutation( BidirIt first, BidirIt last ); | (1) | (constexpr since C++20) |
| template< class BidirIt, class Compare >  bool next_permutation( BidirIt first, BidirIt last, Compare comp ); | (2) | (constexpr since C++20) |
|  |  |  |
...
|  |
| --- |
| ```text template<class BidirIt> bool next_permutation(BidirIt first, BidirIt last) {     auto r_first = std::make_reverse_iterator(last);     auto r_last = std::make_reverse_iterator(first);     auto left = std::is_sorted_until(r_first, r_last);       if (left != r_last)     {         auto right = std::upper_bound(r_first, left, *left);         std::iter_swap(left, right);     }       std::reverse(left.base(), last);     return left != r_last; } ``` |
...
Retrieved from "<https://en.cppreference.com/mwiki/index.php?title=cpp/algorithm/next_permutation&oldid=170758>"
...
- This page was last modified on 9 April 2024, at 22:29.