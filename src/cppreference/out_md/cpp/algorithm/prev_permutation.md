...
| template< class BidirIt >  bool prev_permutation( BidirIt first, BidirIt last ); | (1) | (constexpr since C++20) |
| template< class BidirIt, class Compare >  bool prev_permutation( BidirIt first, BidirIt last, Compare comp ); | (2) | (constexpr since C++20) |
|  |  |  |
...
1) The set of all permutations is ordered lexicographically with respect to operator<(until C++20)std::less{}(since C++20).2) The set of all permutations is ordered lexicographically with respect to comp.
...
|  |
| --- |
| ```text template<class BidirIt> bool prev_permutation(BidirIt first, BidirIt last) {     if (first == last)         return false;     BidirIt i = last;     if (first == --i)         return false;       while (1)     {         BidirIt i1, i2;           i1 = i;         if (*i1 < *--i)         {             i2 = last;             while (!(*--i2 < *i))                 ;             std::iter_swap(i, i2);             std::reverse(i1, last);             return true;         }           if (i == first)         {             std::reverse(first, last);             return false;         }     } } ``` |
...
Retrieved from "<https://en.cppreference.com/mwiki/index.php?title=cpp/algorithm/prev_permutation&oldid=170759>"
...
- This page was last modified on 9 April 2024, at 22:32.