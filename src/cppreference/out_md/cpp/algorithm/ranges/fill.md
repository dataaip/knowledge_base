...
| template< class O, std::sentinel_for<O> S, class T = std::iter_value_t<O> >  requires std::output_iterator<O, const T&> constexpr O fill( O first, S last, const T& value ); |  | (since C++26) |
|  |  |  |
| --- | --- | --- |
|  | (2) |  |
| template< class T, ranges::output_range<const T&> R >  constexpr ranges::borrowed_iterator_t<R> fill( R&& r, const T& value ); |  | (since C++20)  (until C++26) |
| template< class R, class T = ranges::range_value_t<R> >  requires ranges::output_range<R, const T&> constexpr ranges::borrowed_iterator_t<R> fill( R&& r, const T& value ); |  | (since C++26) |
|  |  |  |

1) Assigns the given value to the elements in the range ``first`,`last`)`.2) Same as (1), but uses r as the source range, as if using [ranges::begin(r) as first and ranges::end(r) as last.
...
Retrieved from "<https://en.cppreference.com/mwiki/index.php?title=cpp/algorithm/ranges/fill&oldid=175609>"
...
- This page was last modified on 30 August 2024, at 16:37.