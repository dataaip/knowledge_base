...
| template< class ExecutionPolicy, class BidirIt, class UnaryPred >  BidirIt stable_partition( ExecutionPolicy&& policy,                           BidirIt first, BidirIt last, UnaryPred p ); | (2) | (since C++17) |
|  |  |  |

1) Reorders the elements in the range ``first`,`last`)` in such a way that all elements for which the predicate p returns true precede the elements for which predicate p returns false. Relative order of the elements is preserved.2) Same as (1), but executed according to policy. This overload participates in overload resolution only if all following conditions are satisfied:
...
| - `BidirIt` is not ValueSwappable. - The type of \*first is not MoveConstructible. - The type of \*first is not MoveAssignable. | (since C++11) |
...
1) Exactly \(\scriptsize N\)N applications of p. \(\scriptsize O(N)\)O(N) swaps if there is enough extra memory, otherwise at most \(\scriptsize N \cdot log_{2}(N)\)N⋅log2(N) swaps.2) \(\scriptsize O(N)\)O(N) applications of p. \(\scriptsize N \cdot log(N)\)N⋅log(N) swaps.
...
| --- | --- | --- | --- |
| LWG 2150 | C++98 | `std::stable_partition` was only required to place one element satisfying p before one element not satisfying p | corrected the requirement |
...
- This page was last modified on 10 July 2024, at 04:08.