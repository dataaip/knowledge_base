...
1) Used to obtain the iterator type of the type `T`.2) Used to obtain the constant iterator type of the `range` type `R`.3) Used to obtain the sentinel type of the range type `R`.4) Used to obtain the constant sentinel type of the range type `R`.
...
| --- | --- | --- | --- |
| LWG 3946 | C++23 | `const_iterator_t` and `const_sentinel_t` were inconsistent with the result of `ranges::cbegin` and `ranges::cend` respectively | tweaked |
...
- This page was last modified on 29 August 2024, at 05:10.