...
1) Equivalent to return x.current_ == y.current_;, where `current_` is the underlying iterator.2) Equivalent to return x.current_ < y.current_;, where `current_` is the underlying iterator.3) Equivalent to return y < x;4) Equivalent to return !(y < x);5) Equivalent to return !(x < y);6) Equivalent to return x.current_ <=> y.current_;, where `current_` is the underlying iterator.
...
|  |  |
| --- | --- |
| operator==(C++20) | compares a sentinel with an iterator returned from transform_view::begin   (function) |

Retrieved from "<https://en.cppreference.com/mwiki/index.php?title=cpp/ranges/transform_view/iterator/operator_cmp&oldid=172697>"
...
- This page was last modified on 20 June 2024, at 22:17.