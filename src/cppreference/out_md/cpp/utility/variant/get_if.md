...
1) Index-based non-throwing accessor: If pv is not a null pointer and pv->index() == I, returns a pointer to the value stored in the variant pointed to by pv. Otherwise, returns a null pointer value. The call is ill-formed if `I` is not a valid index in the variant.2) Type-based non-throwing accessor: Equivalent to (1) with `I` being the zero-based index of `T` in Types.... The call is ill-formed if `T` is not a unique element of Types....
...
- This page was last modified on 25 June 2024, at 15:39.