...
- Calling non-const member functions on the string, excluding [`operator[]()`](operator_at.html "cpp/string/basic string/operator at"), at(), front(), back(), begin(), end(), rbegin(), rend().

1) Modifying the character array accessed through the const overload of `data` has undefined behavior.2) Modifying the past-the-end null terminator stored at `data() +` size() to any value other than `CharT()` has undefined behavior.
...
| data | returns a pointer to the first character of a view   (public member function of `std::basic_string_view<CharT,Traits>`) |
...
- This page was last modified on 22 April 2024, at 05:40.