...
| template< class SeedSeq >   void seed( SeedSeq& seq ); | (2) | (since C++26) |
|  |  |  |

Sets the state of the random-number engine.

1) Right after the state is set, \*this == std::philox_engine(value) is true.2) Right after the state is set, \*this == std::philox_engine(seq) is true.
...
1) Same as std::philox_engine(value).2) Same as std::philox_engine(seq).
...
Retrieved from "<https://en.cppreference.com/mwiki/index.php?title=cpp/numeric/random/philox_engine/seed&oldid=175508>"
...
- This page was last modified on 26 August 2024, at 00:05.