...
1-3,5-7) Multiplies a floating-point value arg by FLT_RADIX raised to power exp.4,8) Type-generic macros: If arg has type long double, `scalbnl` or `scalblnl` is called. Otherwise, if arg has integer type or the type double, `scalbn` or `scalbln` is called. Otherwise, `scalbnf` or `scalblnf` is called, respectively.
...
- This page was last modified on 23 May 2024, at 19:51.