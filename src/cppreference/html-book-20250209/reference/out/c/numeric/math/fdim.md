...
1-3) Returns the positive difference between x and y, that is, if x>y, returns x-y, otherwise (if x≤y), returns +0.4) Type-generic macro: If any argument has type long double, `fdiml` is called. Otherwise, if any argument has integer type or has type double, `fdim` is called. Otherwise, `fdimf` is called.
