...
| template< class ForwardIt, class Compare >  ForwardIt is_sorted_until( ForwardIt first, ForwardIt last,                            Compare comp ); | (3) | (since C++11)  (constexpr since C++20) |
...
1) Finds the largest range whether elements are [sorted with respect to operator<(until C++20)std::less{}(since C++20).3) Finds the largest range whether elements are sorted with respect to comp.2,4) Same as (1,3), but executed according to policy. These overloads participate in overload resolution only if all following conditions are satisfied:
...
1,2) \(\scriptsize O(N)\)O(N) comparisons using operator<(until C++20)std::less{}(since C++20).3,4) \(\scriptsize O(N)\)O(N) applications of the comparator comp.
...
| is_sorted_until (1) |
| --- |
| ```text template<class ForwardIt> constexpr //< since C++20 ForwardIt is_sorted_until(ForwardIt first, ForwardIt last) {     return std::is_sorted_until(first, last, std::less<>()); } ``` |
| is_sorted_until (2) |
| ```text template<class ForwardIt, class Compare> constexpr //< since C++20 ForwardIt is_sorted_until(ForwardIt first, ForwardIt last, Compare comp) {     if (first != last)     {         ForwardIt next = first;         while (++next != last)         {             if (comp(*next, *first))                 return next;             first = next;         }     }     return last; } ``` |
...
Retrieved from "<https://en.cppreference.com/mwiki/index.php?title=cpp/algorithm/is_sorted_until&oldid=170573>"
...
- This page was last modified on 2 April 2024, at 22:14.