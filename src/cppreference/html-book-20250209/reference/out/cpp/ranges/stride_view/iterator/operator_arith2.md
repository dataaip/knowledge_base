...
| [stride_view::**iterator**::operator[]](operator_at.html "cpp/ranges/stride view/iterator/operator at") | | | | |
...
1,2) Equivalent to auto r = i; r += n; return r;.3) Equivalent to auto r = i; r -= n; return r;.4) Let `N` be x.current_ - y.current_. Returns:

- (N + x.missing_ - y.missing_) / x.stride_, if `Base` models `forward_range`.
- -/\*div-ceil\*/(-N, x.stride_), if `N < 0`.
- /\*div-ceil\*/(N, x.stride_) otherwise.
5) Equivalent to return /\*div-ceil\*/(x.end_ - x.current_, x.stride_);.6) Equivalent to return -(s - x);.
...
1,2) An incremented iterator3) A decremented iterator4) A distance between given iterators5,6) A distance between given iterator and sentinel
...
Retrieved from "<https://en.cppreference.com/mwiki/index.php?title=cpp/ranges/stride_view/iterator/operator_arith2&oldid=172962>"
...
- This page was last modified on 28 June 2024, at 18:57.