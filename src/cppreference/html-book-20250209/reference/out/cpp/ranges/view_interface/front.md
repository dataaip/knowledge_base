...
|  |  |  |
| --- | --- | --- |
| constexpr decltype(auto) front()      requires ranges::forward_range<D>; | (1) | (since C++20) |
| constexpr decltype(auto) front() const      requires ranges::forward_range<const D>; | (2) | (since C++20) |
|  |  |  |
...
1) Let `derived` be static_cast<D&>(\*this). Equivalent to return \*ranges::begin(derived);. The behavior is undefined if `empty()` is true (i.e. the beginning iterator compares equal to the sentinel), even if the iterator obtained in the same way is dereferenceable.2) Same as (1), except that `derived` is static_cast<const D&>(\*this).
...
Retrieved from "<https://en.cppreference.com/mwiki/index.php?title=cpp/ranges/view_interface/front&oldid=149659>"
...
- This page was last modified on 24 March 2023, at 04:15.