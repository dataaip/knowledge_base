...
|  |  |  |
| --- | --- | --- |
| template< class CharT, class Traits >  friend std::basic_ostream<CharT,Traits>&     operator<<( std::basic_ostream<CharT,Traits>& os, const path& p ); | (1) | (since C++17) |
| template< class CharT, class Traits >  friend std::basic_istream<CharT,Traits>&     operator>>( std::basic_istream<CharT,Traits>& is, path& p ); | (2) | (since C++17) |
|  |  |  |
...
| operator<< |
| --- |
| ```text template<class CharT, class Traits> friend std::basic_ostream<CharT,Traits>&     operator<<(std::basic_ostream<CharT,Traits>& os, const path& p) {     os << std::quoted(p.string<CharT,Traits>());     return os; } ``` |
| operator>> |
| ```text template<class CharT, class Traits> friend std::basic_istream<CharT,Traits>&     operator>>(std::basic_istream<CharT,Traits>& is, path& p) {     std::basic_string<CharT, Traits> t;     is >> std::quoted(t);     p = t;     return is; } ``` |
...
- This page was last modified on 12 July 2023, at 03:03.