...
1) Copy constructor. Initializes the contents of the new `future_error` object with those of other. If \*this and other both have dynamic type `std::future_error` then std::strcmp(what(), other.what()) == 0.2) Constructs a new `future_error` object containing the error code std::make_error_code(ec).
...
- This page was last modified on 11 October 2023, at 10:28.