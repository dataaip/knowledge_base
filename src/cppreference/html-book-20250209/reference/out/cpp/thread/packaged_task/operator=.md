...
1) Copy assignment operator is deleted, `std::packaged_task` is move-only.2) Releases the shared state, if any, destroys the previously-held task, and moves the shared state and the task owned by rhs into \*this. rhs is left without a shared state and with a moved-from task.
...
Retrieved from "<https://en.cppreference.com/mwiki/index.php?title=cpp/thread/packaged_task/operator%3D&oldid=156961>"
...
- This page was last modified on 17 August 2023, at 18:44.