...
2,3) Constructs or destroys an object using the outermost allocator of \*this. Given the type std::allocator_traits<std::remove_reference_t<decltype(`outermost` ﻿())>> as `outermost_traits`:2) Equivalent to outermost_traits::construct(`outermost` ﻿(), p, std::forward<Args>(args)...);.3) Equivalent to outermost_traits::destroy(`outermost` ﻿(), p);.
...
| --- | --- | --- | --- |
| LWG 3116 | C++11 | the recursion condition for `outermost` was “alloc has the member function `outer_allocator()`” | changed to “the expression alloc.outer_allocator() is valid” |
...
Retrieved from "<https://en.cppreference.com/mwiki/index.php?title=cpp/memory/scoped_allocator_adaptor/helpers&oldid=177635>"
...
- This page was last modified on 13 November 2024, at 01:59.