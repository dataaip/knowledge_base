...
1,2) Returns the smaller of a and b.1) Uses operator< to compare the values. If `T` is not LessThanComparable, the behavior is undefined.2) Use the comparison function comp to compare the values.3,4) Returns the smallest of the values in initializer list ilist. If ilist.size() is zero, or `T` is not CopyConstructible, the behavior is undefined.3) Uses operator< to compare the values. If `T` is not LessThanComparable, the behavior is undefined.4) Use the comparison function comp to compare the values.
...
1,2) The smaller of a and b. If the values are equivalent, returns a.3,4) The smallest value in ilist. If several values are equivalent to the smallest, returns the leftmost such value.
...
| min (1) |
| --- |
| ```text template<class T> const T& min(const T& a, const T& b) {     return (b < a) ? b : a; } ``` |
| min (2) |
| ```text template<class T, class Compare> const T& min(const T& a, const T& b, Compare comp) {     return (comp(b, a)) ? b : a; } ``` |
| min (3) |
| ```text template<class T> T min(std::initializer_list<T> ilist) {     return *std::min_element(ilist.begin(), ilist.end()); } ``` |
| min (4) |
| ```text template<class T, class Compare> T min(std::initializer_list<T> ilist, Compare comp) {     return *std::min_element(ilist.begin(), ilist.end(), comp); } ``` |
