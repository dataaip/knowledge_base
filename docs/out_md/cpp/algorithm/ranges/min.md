...
1) Returns the smaller of a and b.2) Returns the first smallest element in the initializer list r.3) Returns the first smallest value in the range r.
...
1) The smaller of a and b, according to the projection. If they are equivalent, returns a.2,3) The smallest element in r, according to the projection. If several values are equivalent to the smallest, returns the leftmost one. If the range is empty (as determined by ranges::distance(r)), the behavior is undefined.
...
Retrieved from "<https://en.cppreference.com/mwiki/index.php?title=cpp/algorithm/ranges/min&oldid=166088>"
...
- This page was last modified on 17 December 2023, at 10:26.