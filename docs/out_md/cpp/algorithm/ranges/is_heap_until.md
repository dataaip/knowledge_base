...
| Call signature |  |  |
| template< std::random_access_iterator I, std::sentinel_for<I> S,  class Proj = std::identity,            std::indirect_strict_weak_order                < std::projected<I, Proj>> Comp = ranges::less > constexpr I is_heap_until( I first, S last, Comp comp = {}, Proj proj = {} ); | (1) | (since C++20) |
| template< ranges::random_access_range R, class Proj = std::identity,  std::indirect_strict_weak_order                <std::projected                     <ranges::iterator_t<R>, Proj>> Comp = ranges::less >  constexpr ranges::borrowed_iterator_t<R>     is_heap_until( R&& r, Comp comp = {}, Proj proj = {} ); | (2) | (since C++20) |
|  |  |  |
...
1) The range ``first`,`iter`)` is a heap with respect to comp and proj.2) The range `[`[ranges::begin(r)`,`iter`)` is a heap with respect to comp and proj.
...
Retrieved from "<https://en.cppreference.com/mwiki/index.php?title=cpp/algorithm/ranges/is_heap_until&oldid=177083>"
...
- This page was last modified on 15 October 2024, at 21:54.