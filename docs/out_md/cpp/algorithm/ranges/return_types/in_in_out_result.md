...
| template< class I1, class I2, class O >  struct in_in_out_result; |  | (since C++20) |
|  |  |  |
...
| in1 | a value (that is supposed to be an iterator) of type `I1`.   (public member object) |
| in2 | a value (that is supposed to be an iterator) of type `I2`.   (public member object) |
| out | a value (that is supposed to be an iterator) of type `O`.   (public member object) |
...
|  |  |  |
| --- | --- | --- |
| template<class II1, class II2, class OO>  requires std::convertible_to<const I1&, II1> &&           std::convertible_to<const I2&, II2> &&           std::convertible_to<const O&, OO> constexpr operator in_in_out_result<II1, II2, OO>() const &; | (1) |  |
| template<class II1, class II2, class OO>  requires std::convertible_to<I1, II1> &&           std::convertible_to<I2, II2> &&           std::convertible_to<O, OO> constexpr operator in_in_out_result<II1, II2, OO>() &&; | (2) |  |
|  |  |  |

Converts \*this to the result by constructing every data member of the result from the corresponding member of \*this.

1) Equivalent to return {in1, in2, out};.2) Equivalent to return {std::move(in1), std::move(in2), std::move(out)};.
...
Retrieved from "<https://en.cppreference.com/mwiki/index.php?title=cpp/algorithm/ranges/return_types/in_in_out_result&oldid=170867>"
...
- This page was last modified on 13 April 2024, at 01:58.