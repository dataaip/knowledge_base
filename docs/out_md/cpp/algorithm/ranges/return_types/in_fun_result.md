...
| template< class I, class F >  struct in_fun_result; |  | (since C++20) |
|  |  |  |
...
| fun | a value (that is supposed to be a function object) of type `F`.   (public member object) |
...
|  |  |  |
| --- | --- | --- |
| template<class I2, class F2>  requires std::convertible_to<const I&, I2> && std::convertible_to<const F&, F2> constexpr operator in_fun_result<I2, F2>() const &; | (1) |  |
| template<class I2, class F2>  requires std::convertible_to<I, I2> && std::convertible_to<F, F2> constexpr operator in_fun_result<I2, F2>() &&; | (2) |  |
|  |  |  |

Converts \*this to the result by constructing every data member of the result from the corresponding member of \*this.

1) Equivalent to return {in, fun};.2) Equivalent to return {std::move(in), std::move(fun)};.
...
Retrieved from "<https://en.cppreference.com/mwiki/index.php?title=cpp/algorithm/ranges/return_types/in_fun_result&oldid=171291>"
...
- This page was last modified on 25 April 2024, at 07:57.