...
| template< class O, class T >  struct out_value_result; |  | (since C++23) |
|  |  |  |
...
| value | a value (that is supposed to be a stored value) of type `T`.   (public member object) |
...
|  |  |  |
| --- | --- | --- |
| template<class O2, class T2>  requires convertible_to<const O&, O2> && convertible_to<const T&, T2> constexpr operator out_value_result<O2, T2>() const &; | (1) |  |
| template<class O2, class T2>  requires convertible_to<O, O2> && convertible_to<T, T2> constexpr operator out_value_result<O2, T2>() &&; | (2) |  |
|  |  |  |

Converts \*this to the result by constructing every data member of the result from the corresponding member of \*this.

1) Equivalent to return {out, value};.2) Equivalent to return {std::move(out), std::move(value)};.
...
Retrieved from "<https://en.cppreference.com/mwiki/index.php?title=cpp/algorithm/ranges/return_types/out_value_result&oldid=170872>"
...
- This page was last modified on 13 April 2024, at 02:03.