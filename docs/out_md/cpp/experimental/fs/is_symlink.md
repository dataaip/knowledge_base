...
| bool is_symlink( const path& p );  bool is_symlink( const path& p, error_code& ec ); | (2) | (filesystem TS) |
|  |  |  |

Checks if the given file status or path corresponds to a symbolic link, as if determined by the POSIX S_IFLNK.

1) Equivalent to s.type() == file_type::symlink.2) Equivalent to is_symlink(status(p)) or is_symlink(status(p, ec)).
...
Retrieved from "<https://en.cppreference.com/mwiki/index.php?title=cpp/experimental/fs/is_symlink&oldid=169731>"
