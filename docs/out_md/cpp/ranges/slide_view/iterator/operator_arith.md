...
Before the invocation, the `current_` and `last_ele_` (if present) must be incrementable.2) Equivalent to: auto tmp = \*this; ++\*this; return tmp;3) Equivalent to:
...
Before the invocation, the `current_` and `last_ele_` (if present) must be decrementable.4) Equivalent to: auto tmp = \*this; --\*this; return tmp;5) Equivalent to:
...
Before the invocation, the expressions current_ + n and last_ele_ + n (if `last_ele_` is present) must have well-defined behavior.6) Equivalent to:
...
- This page was last modified on 10 May 2023, at 13:58.