...
1) Constructs the exception object with what_arg as explanatory string. After construction, std::strcmp(what(), what_arg.c_str()) == 0.2) Constructs the exception object with what_arg as explanatory string. After construction, std::strcmp(what(), what_arg) == 0.3) Copy constructor. If \*this and other both have dynamic type `std::domain_error` then std::strcmp(what(), other.what()) == 0. No exception can be thrown from the copy constructor.
...
| LWG 471 | C++98 | the explanatory strings of `std::domain_error`'s copies were implementation-defined | they are the same as that of the original `std::domain_error` object |

Retrieved from "<https://en.cppreference.com/mwiki/index.php?title=cpp/error/domain_error&oldid=158787>"
...
- This page was last modified on 11 September 2023, at 10:13.