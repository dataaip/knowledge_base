...
1) Calls setbuf(s, n) of the most derived class.2) The base class version of this function has no effect. The derived classes may override this function to allow removal or replacement of the controlled character sequence (the buffer) with a user-provided array, or for any other implementation-specific purpose.
...
1) The return value of setbuf(s, n).2) this
...
| --- | --- | --- | --- |
| LWG 158 | C++98 | the default behavior of `setbuf` was only specified if gptr() is not null and not equal to egptr() | specified as no-op for all cases |
...
- This page was last modified on 29 September 2023, at 23:59.