...
| template<  class CharT,      class Traits = std::char_traits<CharT>  > class basic_ospanstream : public basic_ostream<CharT, Traits> |  | (since C++23) |
|  |  |  |
...
|  |  |
| --- | --- |
| (constructor) | constructs the `basic_ospanstream`   (public member function) |
| ****(destructor)****[virtual] | destroys the `basic_ospanstream`   (virtual public member function) |
| operator= | move-assignments the `basic_ospanstream`   (public member function) |
...
Retrieved from "<https://en.cppreference.com/mwiki/index.php?title=cpp/io/basic_ospanstream&oldid=177367>"
