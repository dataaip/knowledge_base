...
|  |  |  |
| --- | --- | --- |
| template< class T >  constexpr bool is_object_v = is_object<T>::value; |  | (since C++17) |
|  |  |  |
...
|  |  |
| --- | --- |
| value[static] | true if `T` is an object type, false otherwise   (public static member constant) |
...
|  |
| --- |
| ```text template<class T> struct is_object : std::integral_constant<bool,                        std::is_scalar<T>::value ||                        std::is_array<T>::value ||                        std::is_union<T>::value ||                        std::is_class<T>::value> {}; ``` |
...
Retrieved from "<https://en.cppreference.com/mwiki/index.php?title=cpp/types/is_object&oldid=152447>"
...
- This page was last modified on 28 May 2023, at 05:05.