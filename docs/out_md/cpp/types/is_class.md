...
|  |  |  |
| --- | --- | --- |
| template< class T >  constexpr bool is_class_v = is_class<T>::value; |  | (since C++17) |
|  |  |  |
...
|  |  |
| --- | --- |
| value[static] | true if `T` is a non-union class type, false otherwise   (public static member constant) |
...
|  |
| --- |
| ```text namespace detail {     template<class T>     std::integral_constant<bool, !std::is_union<T>::value> test(int T::*);       template<class>     std::false_type test(...); }   template<class T> struct is_class : decltype(detail::test<T>(nullptr)) {}; ``` |
...
Retrieved from "<https://en.cppreference.com/mwiki/index.php?title=cpp/types/is_class&oldid=176478>"
...
- This page was last modified on 24 September 2024, at 14:31.