...
1-3) Computes (x \* y) + z as if to infinite precision and rounded only once to fit the result type.4-6) If the macro constants `FP_FAST_FMA`, `FP_FAST_FMAF`, or `FP_FAST_FMAL` are defined, the corresponding function `fma`, `fmaf`, or `fmal` evaluates faster (in addition to being more precise) than the expression x \* y + z for double, float, and long double arguments, respectively. If defined, these macros evaluate to integer 1.7) Type-generic macro: If any argument has type long double, `fmal` is called. Otherwise, if any argument has integer type or has type double, `fma` is called. Otherwise, `fmaf` is called.
...
Retrieved from "<https://en.cppreference.com/mwiki/index.php?title=c/numeric/math/fma&oldid=172014>"
...
- This page was last modified on 23 May 2024, at 03:48.