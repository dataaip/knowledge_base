...
1-3) Computes the natural (base e) logarithm of 1 + arg. This function is more precise than the expression log(1 + arg) if arg is close to zero.4) Type-generic macro: If arg has type long double, `log1pl` is called. Otherwise, if arg has integer type or the type double, `log1p` is called. Otherwise, `log1pf` is called.
...
- This page was last modified on 23 May 2024, at 07:40.