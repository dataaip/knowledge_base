...
1-3) Rounds the floating-point argument arg to an integer value in floating-point format, using the current rounding mode.5-7, 9-11) Rounds the floating-point argument arg to an integer value in integer format, using the current rounding mode.4,8,12) Type-generic macros: If arg has type long double, `rintl`, `lrintl`, `llrintl` is called. Otherwise, if arg has integer type or the type double, `rint`, `lrint`, `llrint` is called. Otherwise, `rintf`, `lrintf`, `llrintf` is called, respectively.
...
- This page was last modified on 23 May 2024, at 19:47.