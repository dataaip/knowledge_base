...
1-3) Computes 2 raised to the given power `n`.4) Type-generic macro: If `n` has type long double, `exp2l` is called. Otherwise, if `n` has integer type or the type double, `exp2` is called. Otherwise, `exp2f` is called.
...
- This page was last modified on 23 May 2024, at 03:40.