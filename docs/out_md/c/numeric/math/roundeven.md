...
1-3) Computes the nearest integer value to arg (in floating-point format), rounding halfway cases to nearest even integer, regardless of the current rounding mode.4) Type-generic macro: If arg has type long double, `roundevenl` is called. Otherwise, if arg has integer type or the type double, `roundeven` is called. Otherwise, `roundevenf` is called, respectively.
