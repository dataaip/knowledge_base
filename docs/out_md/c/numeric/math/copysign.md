...
1-3) Composes a floating-point value with the magnitude of `x` and the sign of `y`.4) Type-generic macro: If any argument has type long double, `copysignl` is called. Otherwise, if any argument has integer type or has type double, `copysign` is called. Otherwise, `copysignf` is called.
...
- This page was last modified on 23 May 2024, at 03:34.