...
/*
在 C23 标准之前，C 标准允许有符号整数使用多种表示方式，如补码、反码和原码。任意 N 位有符号整数的最小保证范围为 - (2^(N-1) - 1) 到 +(2^(N-1) - 1)。例如，对于一个 8 位有符号整数，范围为 -127 到 127。这种范围适用于反码或原码表示
然而，几乎所有现代数据模型（包括 ILP32、LP32、LP64、LLP64）和大多数 C 编译器（已知的唯一例外是 UNISYS 的一些编译器）都使用补码表示
从 C23 标准开始，C 语言仅允许使用补码表示，且保证的范围为 -2^(N-1) 到 +(2^(N-1) - 1)。例如，8 位有符号整数的范围为 -128 到 127
*/
